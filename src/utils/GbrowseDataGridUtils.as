package utils
{
	...
	public class GbrowseDataGridUtils
	{
		...
		public static function loadDataGridInAgrigo( dataGrid:AdvancedDataGrid ):void 
		{
			...
			var url:String = Consts.SERVER_URL + Consts.AGRIGO_EXPORT_SCRIPT;
		...
		public static function loadDataGridInpyExcel( dataGrid:AdvancedDataGrid ):void 
		{
			//Pass the htmltable in a variable so that it can be delivered
			//to the backend script
			var variables:URLVariables = new URLVariables(); 
			...
			//Setup a new request and make sure that we are 
			//sending the data through a post
			var url:String = Consts.SERVER_URL + Consts.PYGBROWSE_EXPORT_SCRIPT;
			var u:URLRequest = new URLRequest( url );
			u.data = variables; //Pass the variables
			u.method = URLRequestMethod.POST; //Don't forget that we need to send as POST
		...
		public static function loadDataGridInExcelGFF3( dataGrid:AdvancedDataGrid ):void 
		{
			//Pass the htmltable in a variable so that it can be delivered
			//to the backend script
			var variables:URLVariables = new URLVariables(); 
